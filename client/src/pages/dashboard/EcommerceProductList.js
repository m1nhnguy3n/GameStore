              ...
              <Table>
                ...
                <TableBody>
                  {filteredProducts.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                    const { id, productName, image, price, createdAt, stockStatus } = row;
                    ...
                    return (
                      ...
                      >
                        <TableCell padding="checkbox">
                          <Checkbox checked={isItemSelected} onClick={() => handleClick(productName)} />
                        </TableCell>
                        ...
                        <TableCell style={{ minWidth: 160 }}>
                          <Label
                            ...
                            color={
                              (stockStatus.statusName === 'Out of Stock' && 'error') ||
                              (stockStatus.statusName === 'low_stock' && 'warning') ||
                              'success'
                            }
                          >
                            {stockStatus.statusName ? sentenceCase(stockStatus.statusName) : ''}
                          </Label>
                        </TableCell>
                        <TableCell align="right">{fCurrency(price)}</TableCell>
                        <TableCell align="right">
                          <ProductMoreMenu productName={productName} onDelete={() => handleDeleteProduct(id)} />
                        </TableCell>
                      </TableRow>
                    );
                  })}
...
function applySortFilter(array, comparator, query) {
  ...
  if (query) {
    return array.filter((_product) => _product.productName.toLowerCase().indexOf(query.toLowerCase()) !== -1);
  }
